// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using PizzeriaOrder.Api.Data;

#nullable disable

namespace PizzeriaOrder.Api.Migrations
{
    [DbContext(typeof(PizzeriaDbContext))]
    [Migration("20260905074203_AddOrderTables")]
    partial class AddOrderTables
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.11")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Menu.MenuCategory", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("RestaurantId")
                        .HasColumnType("integer");

                    b.Property<int>("SortOrder")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("RestaurantId");

                    b.ToTable("MenuCategories");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            IsActive = true,
                            Name = "Skinkpizzor",
                            RestaurantId = 1,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 2,
                            IsActive = true,
                            Name = "Havspizzor",
                            RestaurantId = 1,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 3,
                            IsActive = true,
                            Name = "Köttfärspizzor",
                            RestaurantId = 1,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 4,
                            IsActive = true,
                            Name = "Salami & Baconpizzor",
                            RestaurantId = 1,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 5,
                            IsActive = true,
                            Name = "Ox-/Fläskfilépizzor",
                            RestaurantId = 1,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 6,
                            IsActive = true,
                            Name = "Gorgonzolapizzor",
                            RestaurantId = 1,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 7,
                            IsActive = true,
                            Name = "Mexikanska pizzor",
                            RestaurantId = 1,
                            SortOrder = 7
                        },
                        new
                        {
                            Id = 8,
                            IsActive = true,
                            Name = "Kycklingpizzor",
                            RestaurantId = 1,
                            SortOrder = 8
                        },
                        new
                        {
                            Id = 9,
                            IsActive = true,
                            Name = "Kebabpizzor",
                            RestaurantId = 1,
                            SortOrder = 9
                        },
                        new
                        {
                            Id = 10,
                            IsActive = true,
                            Name = "Kebab / Kyckling / Falafel",
                            RestaurantId = 1,
                            SortOrder = 10
                        },
                        new
                        {
                            Id = 11,
                            IsActive = true,
                            Name = "Kötträtter",
                            RestaurantId = 1,
                            SortOrder = 11
                        },
                        new
                        {
                            Id = 12,
                            IsActive = true,
                            Name = "Grillrätter",
                            RestaurantId = 1,
                            SortOrder = 12
                        },
                        new
                        {
                            Id = 13,
                            IsActive = true,
                            Name = "Fiskrätter",
                            RestaurantId = 1,
                            SortOrder = 13
                        },
                        new
                        {
                            Id = 14,
                            IsActive = true,
                            Name = "Pastarätter",
                            RestaurantId = 1,
                            SortOrder = 14
                        },
                        new
                        {
                            Id = 15,
                            IsActive = true,
                            Name = "Sallader",
                            RestaurantId = 1,
                            SortOrder = 15
                        },
                        new
                        {
                            Id = 16,
                            IsActive = true,
                            Name = "Barnrätter",
                            RestaurantId = 1,
                            SortOrder = 16
                        });
                });

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Menu.MenuItem", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Description")
                        .HasColumnType("text");

                    b.Property<string>("ImageUrl")
                        .HasColumnType("text");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<int>("MenuCategoryId")
                        .HasColumnType("integer");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<decimal?>("Price")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.Property<int>("SortOrder")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("MenuCategoryId");

                    b.ToTable("MenuItems");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Description = "Ost",
                            IsActive = true,
                            MenuCategoryId = 1,
                            Name = "Margherita",
                            Price = 100m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 2,
                            Description = "Skinka",
                            IsActive = true,
                            MenuCategoryId = 1,
                            Name = "Vesuvio",
                            Price = 100m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 3,
                            Description = "Skinka & ananas",
                            IsActive = true,
                            MenuCategoryId = 1,
                            Name = "Hawaii",
                            Price = 110m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 4,
                            Description = "Skinka & färska champinjoner",
                            IsActive = true,
                            MenuCategoryId = 1,
                            Name = "Capricciosa",
                            Price = 110m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 5,
                            Description = "Skinka & räkor",
                            IsActive = true,
                            MenuCategoryId = 1,
                            Name = "Bussola",
                            Price = 110m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 6,
                            Description = "Inbakad. Skinka",
                            IsActive = true,
                            MenuCategoryId = 1,
                            Name = "Calzone",
                            Price = 110m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 7,
                            Description = "Skinka, ananas, banan & curry",
                            IsActive = true,
                            MenuCategoryId = 1,
                            Name = "Florida",
                            Price = 120m,
                            SortOrder = 7
                        },
                        new
                        {
                            Id = 8,
                            Description = "Skinka, räkor & färska champinjoner",
                            IsActive = true,
                            MenuCategoryId = 1,
                            Name = "Bella",
                            Price = 120m,
                            SortOrder = 8
                        },
                        new
                        {
                            Id = 9,
                            Description = "Skinka, räkor, musslor, oliver, färska champinjoner & kronärtskocka",
                            IsActive = true,
                            MenuCategoryId = 1,
                            Name = "Quattro Stagioni",
                            Price = 125m,
                            SortOrder = 9
                        },
                        new
                        {
                            Id = 10,
                            Description = "Inbakad. Skinka, räkor & färska champinjoner",
                            IsActive = true,
                            MenuCategoryId = 1,
                            Name = "Calzone Special",
                            Price = 120m,
                            SortOrder = 10
                        },
                        new
                        {
                            Id = 11,
                            Description = "Dubbel deg. Skinka, räkor & färska champinjoner",
                            IsActive = true,
                            MenuCategoryId = 1,
                            Name = "Dubbel Calzone",
                            Price = 130m,
                            SortOrder = 11
                        },
                        new
                        {
                            Id = 12,
                            Description = "Skinka & tonfisk",
                            IsActive = true,
                            MenuCategoryId = 2,
                            Name = "Opera",
                            Price = 110m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 13,
                            Description = "Tonfisk & lök",
                            IsActive = true,
                            MenuCategoryId = 2,
                            Name = "Al Tonno",
                            Price = 110m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 14,
                            Description = "Räkor, musslor & tonfisk",
                            IsActive = true,
                            MenuCategoryId = 2,
                            Name = "Tredag",
                            Price = 120m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 15,
                            Description = "Crème fraiche, mozzarella, rödlök, kräftstjärtar & räkor",
                            IsActive = true,
                            MenuCategoryId = 2,
                            Name = "Sjölejon & Iggon Special",
                            Price = 150m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 16,
                            Description = "Scampi, räkor, kräftstjärtar & ruccola",
                            IsActive = true,
                            MenuCategoryId = 2,
                            Name = "Havets Läckerheter",
                            Price = 160m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 17,
                            Description = "Champinjoner, lök, tomat, ananas, feferoni, oliver, paprika & kronärtskocka",
                            IsActive = true,
                            MenuCategoryId = 2,
                            Name = "Vegetariska",
                            Price = 130m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 18,
                            Description = "Mozzarellaost, champinjoner, tomat & ruccolasallad",
                            IsActive = true,
                            MenuCategoryId = 2,
                            Name = "Mozzarella",
                            Price = 120m,
                            SortOrder = 7
                        },
                        new
                        {
                            Id = 19,
                            Description = "Köttfärs & lök",
                            IsActive = true,
                            MenuCategoryId = 3,
                            Name = "Orientale",
                            Price = 110m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 20,
                            Description = "Köttfärs, ägg & tabasco",
                            IsActive = true,
                            MenuCategoryId = 3,
                            Name = "Mafioso",
                            Price = 110m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 21,
                            Description = "Köttfärs, champinjoner & salladsost (feta)",
                            IsActive = true,
                            MenuCategoryId = 3,
                            Name = "Bari",
                            Price = 120m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 22,
                            Description = "Köttfärs, tomat, paprika & lök",
                            IsActive = true,
                            MenuCategoryId = 3,
                            Name = "Orientale Special",
                            Price = 120m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 23,
                            Description = "Skinka, räkor & köttfärs",
                            IsActive = true,
                            MenuCategoryId = 4,
                            Name = "Disco",
                            Price = 125m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 24,
                            Description = "Salami, lök & oliver",
                            IsActive = true,
                            MenuCategoryId = 4,
                            Name = "Campagnola",
                            Price = 110m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 25,
                            Description = "Salami, bacon, lök & ägg",
                            IsActive = true,
                            MenuCategoryId = 4,
                            Name = "Toscana",
                            Price = 120m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 26,
                            Description = "Bacon, vitlök, paprika & lök",
                            IsActive = true,
                            MenuCategoryId = 4,
                            Name = "Verona",
                            Price = 120m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 27,
                            Description = "Skinka & salami",
                            IsActive = true,
                            MenuCategoryId = 4,
                            Name = "Al Capone",
                            Price = 110m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 28,
                            Description = "Skinka, pepperonikorv, gorgonzola & tabasco",
                            IsActive = true,
                            MenuCategoryId = 4,
                            Name = "Björn Special",
                            Price = 120m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 29,
                            Description = "Marinerad oxfilé, lök, färska champinjoner, färska tomater, salladsost (feta), oliver, feferoni & tzatziki",
                            IsActive = true,
                            MenuCategoryId = 5,
                            Name = "Grekisk Pizza",
                            Price = 130m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 30,
                            Description = "Marinerad oxfilé, lök, färsk paprika, färska tomater, färsk persilja & bearnaisesås",
                            IsActive = true,
                            MenuCategoryId = 5,
                            Name = "Trödje Special",
                            Price = 130m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 31,
                            Description = "Marinerad oxfilé, lök, färsk paprika, champinjoner, färska tomater & bearnaisesås",
                            IsActive = true,
                            MenuCategoryId = 5,
                            Name = "Hilleviks Special",
                            Price = 130m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 32,
                            Description = "Marinerad fläskfilé, skinka, färska champinjoner & bearnaisesås",
                            IsActive = true,
                            MenuCategoryId = 5,
                            Name = "Husets Special",
                            Price = 130m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 33,
                            Description = "Marinerad fläskfilé, jalapeño, vitlök, färska champinjoner, färska tomater & bearnaisesås",
                            IsActive = true,
                            MenuCategoryId = 5,
                            Name = "Kockens Special",
                            Price = 130m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 34,
                            Description = "Marinerad fläskfilé, färsk paprika, färska champinjoner & bearnaisesås",
                            IsActive = true,
                            MenuCategoryId = 5,
                            Name = "Oscar",
                            Price = 130m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 35,
                            Description = "Marinerad fläskfilé, skinka, räkor, jalapeño, tomat & bearnaisesås",
                            IsActive = true,
                            MenuCategoryId = 5,
                            Name = "Texas",
                            Price = 130m,
                            SortOrder = 7
                        },
                        new
                        {
                            Id = 36,
                            Description = "Oxfilé, sparris, räkor, färska tomater & bearnaisesås",
                            IsActive = true,
                            MenuCategoryId = 5,
                            Name = "Björke Special",
                            Price = 130m,
                            SortOrder = 8
                        },
                        new
                        {
                            Id = 37,
                            Description = "Fläskfilé, salami, köttfärs, räkor, pommes, färska champinjoner, skinka, kebabkött & valfri sås",
                            IsActive = true,
                            MenuCategoryId = 5,
                            Name = "Vulkan Pizza",
                            Price = 179m,
                            SortOrder = 9
                        },
                        new
                        {
                            Id = 38,
                            Description = "Fläskfilé, paprika, lök, tomat & bearnaisesås",
                            IsActive = true,
                            MenuCategoryId = 5,
                            Name = "Esköns Special",
                            Price = 130m,
                            SortOrder = 10
                        },
                        new
                        {
                            Id = 39,
                            Description = "Oxfilé, lök & gorgonzola",
                            IsActive = true,
                            MenuCategoryId = 6,
                            Name = "Gorgonzola",
                            Price = 125m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 40,
                            Description = "Skinka, lök & gorgonzola",
                            IsActive = true,
                            MenuCategoryId = 6,
                            Name = "Ceasar",
                            Price = 120m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 41,
                            Description = "Skinka, jalapeño, tacokryddmix, tacosås & vitlökssås",
                            IsActive = true,
                            MenuCategoryId = 7,
                            Name = "Azteka",
                            Price = 120m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 42,
                            Description = "Mexikansk köttfärs, lök, vitlök, jalapeño & tacokryddmix",
                            IsActive = true,
                            MenuCategoryId = 7,
                            Name = "Mexikana",
                            Price = 120m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 43,
                            Description = "Marinerad oxfilé, lök, vitlök, jalapeño, färska champinjoner & tacokryddmix",
                            IsActive = true,
                            MenuCategoryId = 7,
                            Name = "Acapulco",
                            Price = 125m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 44,
                            Description = "Kyckling, ananas, banan & curry",
                            IsActive = true,
                            MenuCategoryId = 8,
                            Name = "Indiana",
                            Price = 125m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 45,
                            Description = "Kyckling, jordnötter, banan & curry",
                            IsActive = true,
                            MenuCategoryId = 8,
                            Name = "Kycklingpizza",
                            Price = 125m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 46,
                            Description = "Kyckling, paprika, lök & currysås",
                            IsActive = true,
                            MenuCategoryId = 8,
                            Name = "Hamrånge Special",
                            Price = 125m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 47,
                            Description = "Kebabkött, isbergssallad, rödlök, färska tomater, gurka & feferoni",
                            IsActive = true,
                            MenuCategoryId = 9,
                            Name = "Kebab Special",
                            Price = 140m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 48,
                            Description = "Halvinbakad. Kebabkött, lök, färska tomater, färsk paprika & bearnaisesås",
                            IsActive = true,
                            MenuCategoryId = 9,
                            Name = "Vikingbåt",
                            Price = 130m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 49,
                            Description = "Kebabkött, lök, feferoni & färska tomater",
                            IsActive = true,
                            MenuCategoryId = 9,
                            Name = "Kebabpizza",
                            Price = 130m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 50,
                            Description = "Pommes eller ris, isbergssallad, tomat, gurka & feferoni",
                            IsActive = true,
                            MenuCategoryId = 10,
                            Name = "Kebabtallrik",
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 51,
                            Description = "Färskbakat bröd, isbergssallad, rödlök, tomat, gurka & feferoni",
                            IsActive = true,
                            MenuCategoryId = 10,
                            Name = "Kebabrulle",
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 52,
                            IsActive = true,
                            MenuCategoryId = 10,
                            Name = "Kebabskrovmål",
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 53,
                            Description = "Färskbakat bröd, isbergssallad, rödlök, tomat, gurka & feferoni",
                            IsActive = true,
                            MenuCategoryId = 10,
                            Name = "Kebab med bröd",
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 54,
                            Description = "Med bearnaisesås",
                            IsActive = true,
                            MenuCategoryId = 11,
                            Name = "Oxfilé",
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 55,
                            Description = "Med bearnaisesås",
                            IsActive = true,
                            MenuCategoryId = 11,
                            Name = "Fläskfilé",
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 56,
                            Description = "Med rödvinssås & bearnaisesås",
                            IsActive = true,
                            MenuCategoryId = 11,
                            Name = "Filé Black & White",
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 57,
                            Description = "Fläskfilé, sparris, räkor & bearnaisesås",
                            IsActive = true,
                            MenuCategoryId = 11,
                            Name = "Filé Oscar",
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 58,
                            Description = "Med rödvinssås & bearnaisesås",
                            IsActive = true,
                            MenuCategoryId = 11,
                            Name = "Plankstek",
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 59,
                            Description = "Med bearnaisesås",
                            IsActive = true,
                            MenuCategoryId = 11,
                            Name = "Husets Kycklingfilé",
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 60,
                            Description = "Med tzatziki & tomatsås",
                            IsActive = true,
                            MenuCategoryId = 11,
                            Name = "Kycklingspett",
                            SortOrder = 7
                        },
                        new
                        {
                            Id = 61,
                            Description = "Med bearnaisesås & persiljesmör",
                            IsActive = true,
                            MenuCategoryId = 11,
                            Name = "Schnitzel",
                            SortOrder = 8
                        },
                        new
                        {
                            Id = 62,
                            Description = "Med bearnaisesås & persiljesmör",
                            IsActive = true,
                            MenuCategoryId = 11,
                            Name = "Lövbit",
                            SortOrder = 9
                        },
                        new
                        {
                            Id = 63,
                            Description = "Med tomatsås & tzatziki",
                            IsActive = true,
                            MenuCategoryId = 11,
                            Name = "Fläskspett",
                            SortOrder = 10
                        },
                        new
                        {
                            Id = 64,
                            Description = "90 g hamburgare med bröd",
                            IsActive = true,
                            MenuCategoryId = 12,
                            Name = "Big Star",
                            Price = 89m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 65,
                            Description = "90 g hamburgare med bröd, pommes & dricka 33 cl",
                            IsActive = true,
                            MenuCategoryId = 12,
                            Name = "Big Meal",
                            Price = 119m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 66,
                            Description = "150 g hamburgare med bröd",
                            IsActive = true,
                            MenuCategoryId = 12,
                            Name = "Superstar",
                            Price = 99m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 67,
                            Description = "150 g hamburgare med bröd, pommes & dricka 33 cl",
                            IsActive = true,
                            MenuCategoryId = 12,
                            Name = "Super Meal",
                            Price = 129m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 68,
                            Description = "Chicky bits, pommes, grönsaker, currydressing & dricka 33 cl",
                            IsActive = true,
                            MenuCategoryId = 12,
                            Name = "Chicky Bits Meal",
                            Price = 120m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 69,
                            Description = "Med remouladsås & kokt potatis",
                            IsActive = true,
                            MenuCategoryId = 13,
                            Name = "Rödspätta",
                            Price = 129m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 70,
                            Description = "Ox-/fläskfilé, champinjoner, tomatsås & gräddsås",
                            IsActive = true,
                            MenuCategoryId = 14,
                            Name = "Ox-/Fläskfilé Pasta",
                            Price = 129m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 71,
                            Description = "Kycklingfilé, paprika, lök & currysås",
                            IsActive = true,
                            MenuCategoryId = 14,
                            Name = "Kyckling Pasta",
                            Price = 129m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 72,
                            Description = "Stekta grönsaker & tomatgräddsås",
                            IsActive = true,
                            MenuCategoryId = 14,
                            Name = "Vegetarisk Pasta",
                            Price = 129m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 73,
                            Description = "Bacon, lök, äggula & gräddsås",
                            IsActive = true,
                            MenuCategoryId = 14,
                            Name = "Pasta Carbonara",
                            Price = 129m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 74,
                            Description = "Köttfärssås",
                            IsActive = true,
                            MenuCategoryId = 14,
                            Name = "Pasta Bolognese",
                            Price = 119m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 75,
                            Description = "Avocado, räkor, kaviar & citron",
                            IsActive = true,
                            MenuCategoryId = 15,
                            Name = "Avocadosallad",
                            Price = 120m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 76,
                            Description = "Ost, skinka & ägg",
                            IsActive = true,
                            MenuCategoryId = 15,
                            Name = "Ost & Skinksallad",
                            Price = 120m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 77,
                            Description = "Räkor, ägg, dill & citron",
                            IsActive = true,
                            MenuCategoryId = 15,
                            Name = "Räksallad",
                            Price = 120m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 78,
                            Description = "Tonfisk, rödlök, ägg & citron",
                            IsActive = true,
                            MenuCategoryId = 15,
                            Name = "Tonfisksallad",
                            Price = 120m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 79,
                            Description = "Kryddad salladskycklingfilé & ananas",
                            IsActive = true,
                            MenuCategoryId = 15,
                            Name = "Kycklingsallad",
                            Price = 120m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 80,
                            Description = "Grekisk fårost, salladsost, oliver, feferoni, rödlök, paprika, persilja & citron",
                            IsActive = true,
                            MenuCategoryId = 15,
                            Name = "Grekisk Sallad",
                            Price = 120m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 81,
                            Description = "Skinka, räkor, ost, tonfisk & ägg",
                            IsActive = true,
                            MenuCategoryId = 15,
                            Name = "Blandsallad",
                            Price = 120m,
                            SortOrder = 7
                        },
                        new
                        {
                            Id = 82,
                            Description = "Räkor, tonfisk, musslor, ägg, dill & citron",
                            IsActive = true,
                            MenuCategoryId = 15,
                            Name = "Västkustsallad",
                            Price = 120m,
                            SortOrder = 8
                        },
                        new
                        {
                            Id = 83,
                            Description = "Kebabkött, rödlök, feferoni, persilja & citron",
                            IsActive = true,
                            MenuCategoryId = 15,
                            Name = "Kebabsallad",
                            Price = 120m,
                            SortOrder = 9
                        },
                        new
                        {
                            Id = 84,
                            Description = "45 g hamburgare med bröd & dricka 33 cl",
                            IsActive = true,
                            MenuCategoryId = 16,
                            Name = "Barnbox",
                            Price = 89m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 85,
                            Description = "Chicky bits, pommes & dricka 33 cl",
                            IsActive = true,
                            MenuCategoryId = 16,
                            Name = "Chicky Bits Meal",
                            Price = 89m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 86,
                            Description = "Med sylt & grädde, dricka 33 cl",
                            IsActive = true,
                            MenuCategoryId = 16,
                            Name = "Pannkaka",
                            Price = 89m,
                            SortOrder = 3
                        });
                });

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Menu.MenuItemExtra", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<int>("MenuItemId")
                        .HasColumnType("integer");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<decimal>("PriceChange")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.Property<int>("SortOrder")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("MenuItemId");

                    b.ToTable("MenuItemExtras");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            IsActive = true,
                            MenuItemId = 1,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 2,
                            IsActive = true,
                            MenuItemId = 1,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 3,
                            IsActive = true,
                            MenuItemId = 1,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 4,
                            IsActive = true,
                            MenuItemId = 1,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 5,
                            IsActive = true,
                            MenuItemId = 1,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 6,
                            IsActive = true,
                            MenuItemId = 1,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 7,
                            IsActive = true,
                            MenuItemId = 2,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 8,
                            IsActive = true,
                            MenuItemId = 2,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 9,
                            IsActive = true,
                            MenuItemId = 2,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 10,
                            IsActive = true,
                            MenuItemId = 2,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 11,
                            IsActive = true,
                            MenuItemId = 2,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 12,
                            IsActive = true,
                            MenuItemId = 2,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 13,
                            IsActive = true,
                            MenuItemId = 3,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 14,
                            IsActive = true,
                            MenuItemId = 3,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 15,
                            IsActive = true,
                            MenuItemId = 3,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 16,
                            IsActive = true,
                            MenuItemId = 3,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 17,
                            IsActive = true,
                            MenuItemId = 3,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 18,
                            IsActive = true,
                            MenuItemId = 3,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 19,
                            IsActive = true,
                            MenuItemId = 4,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 20,
                            IsActive = true,
                            MenuItemId = 4,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 21,
                            IsActive = true,
                            MenuItemId = 4,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 22,
                            IsActive = true,
                            MenuItemId = 4,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 23,
                            IsActive = true,
                            MenuItemId = 4,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 24,
                            IsActive = true,
                            MenuItemId = 4,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 25,
                            IsActive = true,
                            MenuItemId = 5,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 26,
                            IsActive = true,
                            MenuItemId = 5,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 27,
                            IsActive = true,
                            MenuItemId = 5,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 28,
                            IsActive = true,
                            MenuItemId = 5,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 29,
                            IsActive = true,
                            MenuItemId = 5,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 30,
                            IsActive = true,
                            MenuItemId = 5,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 31,
                            IsActive = true,
                            MenuItemId = 6,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 32,
                            IsActive = true,
                            MenuItemId = 6,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 33,
                            IsActive = true,
                            MenuItemId = 6,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 34,
                            IsActive = true,
                            MenuItemId = 6,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 35,
                            IsActive = true,
                            MenuItemId = 6,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 36,
                            IsActive = true,
                            MenuItemId = 6,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 37,
                            IsActive = true,
                            MenuItemId = 7,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 38,
                            IsActive = true,
                            MenuItemId = 7,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 39,
                            IsActive = true,
                            MenuItemId = 7,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 40,
                            IsActive = true,
                            MenuItemId = 7,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 41,
                            IsActive = true,
                            MenuItemId = 7,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 42,
                            IsActive = true,
                            MenuItemId = 7,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 43,
                            IsActive = true,
                            MenuItemId = 8,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 44,
                            IsActive = true,
                            MenuItemId = 8,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 45,
                            IsActive = true,
                            MenuItemId = 8,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 46,
                            IsActive = true,
                            MenuItemId = 8,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 47,
                            IsActive = true,
                            MenuItemId = 8,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 48,
                            IsActive = true,
                            MenuItemId = 8,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 49,
                            IsActive = true,
                            MenuItemId = 9,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 50,
                            IsActive = true,
                            MenuItemId = 9,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 51,
                            IsActive = true,
                            MenuItemId = 9,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 52,
                            IsActive = true,
                            MenuItemId = 9,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 53,
                            IsActive = true,
                            MenuItemId = 9,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 54,
                            IsActive = true,
                            MenuItemId = 9,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 55,
                            IsActive = true,
                            MenuItemId = 10,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 56,
                            IsActive = true,
                            MenuItemId = 10,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 57,
                            IsActive = true,
                            MenuItemId = 10,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 58,
                            IsActive = true,
                            MenuItemId = 10,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 59,
                            IsActive = true,
                            MenuItemId = 10,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 60,
                            IsActive = true,
                            MenuItemId = 10,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 61,
                            IsActive = true,
                            MenuItemId = 11,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 62,
                            IsActive = true,
                            MenuItemId = 11,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 63,
                            IsActive = true,
                            MenuItemId = 11,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 64,
                            IsActive = true,
                            MenuItemId = 11,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 65,
                            IsActive = true,
                            MenuItemId = 11,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 66,
                            IsActive = true,
                            MenuItemId = 11,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 67,
                            IsActive = true,
                            MenuItemId = 12,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 68,
                            IsActive = true,
                            MenuItemId = 12,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 69,
                            IsActive = true,
                            MenuItemId = 12,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 70,
                            IsActive = true,
                            MenuItemId = 12,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 71,
                            IsActive = true,
                            MenuItemId = 12,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 72,
                            IsActive = true,
                            MenuItemId = 12,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 73,
                            IsActive = true,
                            MenuItemId = 13,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 74,
                            IsActive = true,
                            MenuItemId = 13,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 75,
                            IsActive = true,
                            MenuItemId = 13,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 76,
                            IsActive = true,
                            MenuItemId = 13,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 77,
                            IsActive = true,
                            MenuItemId = 13,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 78,
                            IsActive = true,
                            MenuItemId = 13,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 79,
                            IsActive = true,
                            MenuItemId = 14,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 80,
                            IsActive = true,
                            MenuItemId = 14,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 81,
                            IsActive = true,
                            MenuItemId = 14,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 82,
                            IsActive = true,
                            MenuItemId = 14,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 83,
                            IsActive = true,
                            MenuItemId = 14,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 84,
                            IsActive = true,
                            MenuItemId = 14,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 85,
                            IsActive = true,
                            MenuItemId = 15,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 86,
                            IsActive = true,
                            MenuItemId = 15,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 87,
                            IsActive = true,
                            MenuItemId = 15,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 88,
                            IsActive = true,
                            MenuItemId = 15,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 89,
                            IsActive = true,
                            MenuItemId = 15,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 90,
                            IsActive = true,
                            MenuItemId = 15,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 91,
                            IsActive = true,
                            MenuItemId = 16,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 92,
                            IsActive = true,
                            MenuItemId = 16,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 93,
                            IsActive = true,
                            MenuItemId = 16,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 94,
                            IsActive = true,
                            MenuItemId = 16,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 95,
                            IsActive = true,
                            MenuItemId = 16,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 96,
                            IsActive = true,
                            MenuItemId = 16,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 97,
                            IsActive = true,
                            MenuItemId = 17,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 98,
                            IsActive = true,
                            MenuItemId = 17,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 99,
                            IsActive = true,
                            MenuItemId = 17,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 100,
                            IsActive = true,
                            MenuItemId = 17,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 101,
                            IsActive = true,
                            MenuItemId = 17,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 102,
                            IsActive = true,
                            MenuItemId = 17,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 103,
                            IsActive = true,
                            MenuItemId = 18,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 104,
                            IsActive = true,
                            MenuItemId = 18,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 105,
                            IsActive = true,
                            MenuItemId = 18,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 106,
                            IsActive = true,
                            MenuItemId = 18,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 107,
                            IsActive = true,
                            MenuItemId = 18,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 108,
                            IsActive = true,
                            MenuItemId = 18,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 109,
                            IsActive = true,
                            MenuItemId = 19,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 110,
                            IsActive = true,
                            MenuItemId = 19,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 111,
                            IsActive = true,
                            MenuItemId = 19,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 112,
                            IsActive = true,
                            MenuItemId = 19,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 113,
                            IsActive = true,
                            MenuItemId = 19,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 114,
                            IsActive = true,
                            MenuItemId = 19,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 115,
                            IsActive = true,
                            MenuItemId = 20,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 116,
                            IsActive = true,
                            MenuItemId = 20,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 117,
                            IsActive = true,
                            MenuItemId = 20,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 118,
                            IsActive = true,
                            MenuItemId = 20,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 119,
                            IsActive = true,
                            MenuItemId = 20,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 120,
                            IsActive = true,
                            MenuItemId = 20,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 121,
                            IsActive = true,
                            MenuItemId = 21,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 122,
                            IsActive = true,
                            MenuItemId = 21,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 123,
                            IsActive = true,
                            MenuItemId = 21,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 124,
                            IsActive = true,
                            MenuItemId = 21,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 125,
                            IsActive = true,
                            MenuItemId = 21,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 126,
                            IsActive = true,
                            MenuItemId = 21,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 127,
                            IsActive = true,
                            MenuItemId = 22,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 128,
                            IsActive = true,
                            MenuItemId = 22,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 129,
                            IsActive = true,
                            MenuItemId = 22,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 130,
                            IsActive = true,
                            MenuItemId = 22,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 131,
                            IsActive = true,
                            MenuItemId = 22,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 132,
                            IsActive = true,
                            MenuItemId = 22,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 133,
                            IsActive = true,
                            MenuItemId = 23,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 134,
                            IsActive = true,
                            MenuItemId = 23,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 135,
                            IsActive = true,
                            MenuItemId = 23,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 136,
                            IsActive = true,
                            MenuItemId = 23,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 137,
                            IsActive = true,
                            MenuItemId = 23,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 138,
                            IsActive = true,
                            MenuItemId = 23,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 139,
                            IsActive = true,
                            MenuItemId = 24,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 140,
                            IsActive = true,
                            MenuItemId = 24,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 141,
                            IsActive = true,
                            MenuItemId = 24,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 142,
                            IsActive = true,
                            MenuItemId = 24,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 143,
                            IsActive = true,
                            MenuItemId = 24,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 144,
                            IsActive = true,
                            MenuItemId = 24,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 145,
                            IsActive = true,
                            MenuItemId = 25,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 146,
                            IsActive = true,
                            MenuItemId = 25,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 147,
                            IsActive = true,
                            MenuItemId = 25,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 148,
                            IsActive = true,
                            MenuItemId = 25,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 149,
                            IsActive = true,
                            MenuItemId = 25,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 150,
                            IsActive = true,
                            MenuItemId = 25,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 151,
                            IsActive = true,
                            MenuItemId = 26,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 152,
                            IsActive = true,
                            MenuItemId = 26,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 153,
                            IsActive = true,
                            MenuItemId = 26,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 154,
                            IsActive = true,
                            MenuItemId = 26,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 155,
                            IsActive = true,
                            MenuItemId = 26,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 156,
                            IsActive = true,
                            MenuItemId = 26,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 157,
                            IsActive = true,
                            MenuItemId = 27,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 158,
                            IsActive = true,
                            MenuItemId = 27,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 159,
                            IsActive = true,
                            MenuItemId = 27,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 160,
                            IsActive = true,
                            MenuItemId = 27,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 161,
                            IsActive = true,
                            MenuItemId = 27,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 162,
                            IsActive = true,
                            MenuItemId = 27,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 163,
                            IsActive = true,
                            MenuItemId = 28,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 164,
                            IsActive = true,
                            MenuItemId = 28,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 165,
                            IsActive = true,
                            MenuItemId = 28,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 166,
                            IsActive = true,
                            MenuItemId = 28,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 167,
                            IsActive = true,
                            MenuItemId = 28,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 168,
                            IsActive = true,
                            MenuItemId = 28,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 169,
                            IsActive = true,
                            MenuItemId = 29,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 170,
                            IsActive = true,
                            MenuItemId = 29,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 171,
                            IsActive = true,
                            MenuItemId = 29,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 172,
                            IsActive = true,
                            MenuItemId = 29,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 173,
                            IsActive = true,
                            MenuItemId = 29,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 174,
                            IsActive = true,
                            MenuItemId = 29,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 175,
                            IsActive = true,
                            MenuItemId = 30,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 176,
                            IsActive = true,
                            MenuItemId = 30,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 177,
                            IsActive = true,
                            MenuItemId = 30,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 178,
                            IsActive = true,
                            MenuItemId = 30,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 179,
                            IsActive = true,
                            MenuItemId = 30,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 180,
                            IsActive = true,
                            MenuItemId = 30,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 181,
                            IsActive = true,
                            MenuItemId = 31,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 182,
                            IsActive = true,
                            MenuItemId = 31,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 183,
                            IsActive = true,
                            MenuItemId = 31,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 184,
                            IsActive = true,
                            MenuItemId = 31,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 185,
                            IsActive = true,
                            MenuItemId = 31,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 186,
                            IsActive = true,
                            MenuItemId = 31,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 187,
                            IsActive = true,
                            MenuItemId = 32,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 188,
                            IsActive = true,
                            MenuItemId = 32,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 189,
                            IsActive = true,
                            MenuItemId = 32,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 190,
                            IsActive = true,
                            MenuItemId = 32,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 191,
                            IsActive = true,
                            MenuItemId = 32,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 192,
                            IsActive = true,
                            MenuItemId = 32,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 193,
                            IsActive = true,
                            MenuItemId = 33,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 194,
                            IsActive = true,
                            MenuItemId = 33,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 195,
                            IsActive = true,
                            MenuItemId = 33,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 196,
                            IsActive = true,
                            MenuItemId = 33,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 197,
                            IsActive = true,
                            MenuItemId = 33,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 198,
                            IsActive = true,
                            MenuItemId = 33,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 199,
                            IsActive = true,
                            MenuItemId = 34,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 200,
                            IsActive = true,
                            MenuItemId = 34,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 201,
                            IsActive = true,
                            MenuItemId = 34,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 202,
                            IsActive = true,
                            MenuItemId = 34,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 203,
                            IsActive = true,
                            MenuItemId = 34,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 204,
                            IsActive = true,
                            MenuItemId = 34,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 205,
                            IsActive = true,
                            MenuItemId = 35,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 206,
                            IsActive = true,
                            MenuItemId = 35,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 207,
                            IsActive = true,
                            MenuItemId = 35,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 208,
                            IsActive = true,
                            MenuItemId = 35,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 209,
                            IsActive = true,
                            MenuItemId = 35,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 210,
                            IsActive = true,
                            MenuItemId = 35,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 211,
                            IsActive = true,
                            MenuItemId = 36,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 212,
                            IsActive = true,
                            MenuItemId = 36,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 213,
                            IsActive = true,
                            MenuItemId = 36,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 214,
                            IsActive = true,
                            MenuItemId = 36,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 215,
                            IsActive = true,
                            MenuItemId = 36,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 216,
                            IsActive = true,
                            MenuItemId = 36,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 217,
                            IsActive = true,
                            MenuItemId = 37,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 218,
                            IsActive = true,
                            MenuItemId = 37,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 219,
                            IsActive = true,
                            MenuItemId = 37,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 220,
                            IsActive = true,
                            MenuItemId = 37,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 221,
                            IsActive = true,
                            MenuItemId = 37,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 222,
                            IsActive = true,
                            MenuItemId = 37,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 223,
                            IsActive = true,
                            MenuItemId = 38,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 224,
                            IsActive = true,
                            MenuItemId = 38,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 225,
                            IsActive = true,
                            MenuItemId = 38,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 226,
                            IsActive = true,
                            MenuItemId = 38,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 227,
                            IsActive = true,
                            MenuItemId = 38,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 228,
                            IsActive = true,
                            MenuItemId = 38,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 229,
                            IsActive = true,
                            MenuItemId = 39,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 230,
                            IsActive = true,
                            MenuItemId = 39,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 231,
                            IsActive = true,
                            MenuItemId = 39,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 232,
                            IsActive = true,
                            MenuItemId = 39,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 233,
                            IsActive = true,
                            MenuItemId = 39,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 234,
                            IsActive = true,
                            MenuItemId = 39,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 235,
                            IsActive = true,
                            MenuItemId = 40,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 236,
                            IsActive = true,
                            MenuItemId = 40,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 237,
                            IsActive = true,
                            MenuItemId = 40,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 238,
                            IsActive = true,
                            MenuItemId = 40,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 239,
                            IsActive = true,
                            MenuItemId = 40,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 240,
                            IsActive = true,
                            MenuItemId = 40,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 241,
                            IsActive = true,
                            MenuItemId = 41,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 242,
                            IsActive = true,
                            MenuItemId = 41,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 243,
                            IsActive = true,
                            MenuItemId = 41,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 244,
                            IsActive = true,
                            MenuItemId = 41,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 245,
                            IsActive = true,
                            MenuItemId = 41,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 246,
                            IsActive = true,
                            MenuItemId = 41,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 247,
                            IsActive = true,
                            MenuItemId = 42,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 248,
                            IsActive = true,
                            MenuItemId = 42,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 249,
                            IsActive = true,
                            MenuItemId = 42,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 250,
                            IsActive = true,
                            MenuItemId = 42,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 251,
                            IsActive = true,
                            MenuItemId = 42,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 252,
                            IsActive = true,
                            MenuItemId = 42,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 253,
                            IsActive = true,
                            MenuItemId = 43,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 254,
                            IsActive = true,
                            MenuItemId = 43,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 255,
                            IsActive = true,
                            MenuItemId = 43,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 256,
                            IsActive = true,
                            MenuItemId = 43,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 257,
                            IsActive = true,
                            MenuItemId = 43,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 258,
                            IsActive = true,
                            MenuItemId = 43,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 259,
                            IsActive = true,
                            MenuItemId = 44,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 260,
                            IsActive = true,
                            MenuItemId = 44,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 261,
                            IsActive = true,
                            MenuItemId = 44,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 262,
                            IsActive = true,
                            MenuItemId = 44,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 263,
                            IsActive = true,
                            MenuItemId = 44,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 264,
                            IsActive = true,
                            MenuItemId = 44,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 265,
                            IsActive = true,
                            MenuItemId = 45,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 266,
                            IsActive = true,
                            MenuItemId = 45,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 267,
                            IsActive = true,
                            MenuItemId = 45,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 268,
                            IsActive = true,
                            MenuItemId = 45,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 269,
                            IsActive = true,
                            MenuItemId = 45,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 270,
                            IsActive = true,
                            MenuItemId = 45,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 271,
                            IsActive = true,
                            MenuItemId = 46,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 272,
                            IsActive = true,
                            MenuItemId = 46,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 273,
                            IsActive = true,
                            MenuItemId = 46,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 274,
                            IsActive = true,
                            MenuItemId = 46,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 275,
                            IsActive = true,
                            MenuItemId = 46,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 276,
                            IsActive = true,
                            MenuItemId = 46,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 277,
                            IsActive = true,
                            MenuItemId = 47,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 278,
                            IsActive = true,
                            MenuItemId = 47,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 279,
                            IsActive = true,
                            MenuItemId = 47,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 280,
                            IsActive = true,
                            MenuItemId = 47,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 281,
                            IsActive = true,
                            MenuItemId = 47,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 282,
                            IsActive = true,
                            MenuItemId = 47,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 283,
                            IsActive = true,
                            MenuItemId = 48,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 284,
                            IsActive = true,
                            MenuItemId = 48,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 285,
                            IsActive = true,
                            MenuItemId = 48,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 286,
                            IsActive = true,
                            MenuItemId = 48,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 287,
                            IsActive = true,
                            MenuItemId = 48,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 288,
                            IsActive = true,
                            MenuItemId = 48,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        },
                        new
                        {
                            Id = 289,
                            IsActive = true,
                            MenuItemId = 49,
                            Name = "Kebab",
                            PriceChange = 25m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 290,
                            IsActive = true,
                            MenuItemId = 49,
                            Name = "Barnstorlek",
                            PriceChange = -10m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 291,
                            IsActive = true,
                            MenuItemId = 49,
                            Name = "Glutenfri",
                            PriceChange = 35m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 292,
                            IsActive = true,
                            MenuItemId = 49,
                            Name = "Extra pålägg",
                            PriceChange = 15m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 293,
                            IsActive = true,
                            MenuItemId = 49,
                            Name = "Pizzasallad",
                            PriceChange = 15m,
                            SortOrder = 5
                        },
                        new
                        {
                            Id = 294,
                            IsActive = true,
                            MenuItemId = 49,
                            Name = "Dressing",
                            PriceChange = 15m,
                            SortOrder = 6
                        });
                });

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Menu.MenuItemVariant", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<int>("MenuItemId")
                        .HasColumnType("integer");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<decimal>("Price")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.Property<int>("SortOrder")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("MenuItemId");

                    b.ToTable("MenuItemVariants");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            IsActive = true,
                            MenuItemId = 50,
                            Name = "Kebab",
                            Price = 120m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 2,
                            IsActive = true,
                            MenuItemId = 50,
                            Name = "Kyckling",
                            Price = 125m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 3,
                            IsActive = true,
                            MenuItemId = 50,
                            Name = "Falafel",
                            Price = 110m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 4,
                            IsActive = true,
                            MenuItemId = 51,
                            Name = "Kebab",
                            Price = 130m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 5,
                            IsActive = true,
                            MenuItemId = 51,
                            Name = "Kyckling",
                            Price = 135m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 6,
                            IsActive = true,
                            MenuItemId = 51,
                            Name = "Falafel",
                            Price = 120m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 7,
                            IsActive = true,
                            MenuItemId = 52,
                            Name = "Kebab",
                            Price = 120m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 8,
                            IsActive = true,
                            MenuItemId = 52,
                            Name = "Kyckling",
                            Price = 125m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 9,
                            IsActive = true,
                            MenuItemId = 52,
                            Name = "Falafel",
                            Price = 120m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 10,
                            IsActive = true,
                            MenuItemId = 53,
                            Name = "Kebab",
                            Price = 110m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 11,
                            IsActive = true,
                            MenuItemId = 53,
                            Name = "Kyckling",
                            Price = 115m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 12,
                            IsActive = true,
                            MenuItemId = 53,
                            Name = "Falafel",
                            Price = 100m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 13,
                            IsActive = true,
                            MenuItemId = 54,
                            Name = "Kokt potatis",
                            Price = 229m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 14,
                            IsActive = true,
                            MenuItemId = 54,
                            Name = "Klyftpotatis",
                            Price = 229m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 15,
                            IsActive = true,
                            MenuItemId = 54,
                            Name = "Pommes",
                            Price = 229m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 16,
                            IsActive = true,
                            MenuItemId = 54,
                            Name = "Ris",
                            Price = 229m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 17,
                            IsActive = true,
                            MenuItemId = 55,
                            Name = "Kokt potatis",
                            Price = 199m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 18,
                            IsActive = true,
                            MenuItemId = 55,
                            Name = "Klyftpotatis",
                            Price = 199m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 19,
                            IsActive = true,
                            MenuItemId = 55,
                            Name = "Pommes",
                            Price = 199m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 20,
                            IsActive = true,
                            MenuItemId = 55,
                            Name = "Ris",
                            Price = 199m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 21,
                            IsActive = true,
                            MenuItemId = 56,
                            Name = "Kokt potatis",
                            Price = 229m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 22,
                            IsActive = true,
                            MenuItemId = 56,
                            Name = "Klyftpotatis",
                            Price = 229m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 23,
                            IsActive = true,
                            MenuItemId = 56,
                            Name = "Pommes",
                            Price = 229m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 24,
                            IsActive = true,
                            MenuItemId = 56,
                            Name = "Ris",
                            Price = 229m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 25,
                            IsActive = true,
                            MenuItemId = 57,
                            Name = "Kokt potatis",
                            Price = 229m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 26,
                            IsActive = true,
                            MenuItemId = 57,
                            Name = "Klyftpotatis",
                            Price = 229m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 27,
                            IsActive = true,
                            MenuItemId = 57,
                            Name = "Pommes",
                            Price = 229m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 28,
                            IsActive = true,
                            MenuItemId = 57,
                            Name = "Ris",
                            Price = 229m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 29,
                            IsActive = true,
                            MenuItemId = 58,
                            Name = "Kokt potatis",
                            Price = 249m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 30,
                            IsActive = true,
                            MenuItemId = 58,
                            Name = "Klyftpotatis",
                            Price = 249m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 31,
                            IsActive = true,
                            MenuItemId = 58,
                            Name = "Pommes",
                            Price = 249m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 32,
                            IsActive = true,
                            MenuItemId = 58,
                            Name = "Ris",
                            Price = 249m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 33,
                            IsActive = true,
                            MenuItemId = 59,
                            Name = "Kokt potatis",
                            Price = 139m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 34,
                            IsActive = true,
                            MenuItemId = 59,
                            Name = "Klyftpotatis",
                            Price = 139m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 35,
                            IsActive = true,
                            MenuItemId = 59,
                            Name = "Pommes",
                            Price = 139m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 36,
                            IsActive = true,
                            MenuItemId = 59,
                            Name = "Ris",
                            Price = 139m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 37,
                            IsActive = true,
                            MenuItemId = 60,
                            Name = "Kokt potatis",
                            Price = 179m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 38,
                            IsActive = true,
                            MenuItemId = 60,
                            Name = "Klyftpotatis",
                            Price = 179m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 39,
                            IsActive = true,
                            MenuItemId = 60,
                            Name = "Pommes",
                            Price = 179m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 40,
                            IsActive = true,
                            MenuItemId = 60,
                            Name = "Ris",
                            Price = 179m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 41,
                            IsActive = true,
                            MenuItemId = 61,
                            Name = "Kokt potatis",
                            Price = 129m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 42,
                            IsActive = true,
                            MenuItemId = 61,
                            Name = "Klyftpotatis",
                            Price = 129m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 43,
                            IsActive = true,
                            MenuItemId = 61,
                            Name = "Pommes",
                            Price = 129m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 44,
                            IsActive = true,
                            MenuItemId = 61,
                            Name = "Ris",
                            Price = 129m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 45,
                            IsActive = true,
                            MenuItemId = 62,
                            Name = "Kokt potatis",
                            Price = 129m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 46,
                            IsActive = true,
                            MenuItemId = 62,
                            Name = "Klyftpotatis",
                            Price = 129m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 47,
                            IsActive = true,
                            MenuItemId = 62,
                            Name = "Pommes",
                            Price = 129m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 48,
                            IsActive = true,
                            MenuItemId = 62,
                            Name = "Ris",
                            Price = 129m,
                            SortOrder = 4
                        },
                        new
                        {
                            Id = 49,
                            IsActive = true,
                            MenuItemId = 63,
                            Name = "Kokt potatis",
                            Price = 209m,
                            SortOrder = 1
                        },
                        new
                        {
                            Id = 50,
                            IsActive = true,
                            MenuItemId = 63,
                            Name = "Klyftpotatis",
                            Price = 209m,
                            SortOrder = 2
                        },
                        new
                        {
                            Id = 51,
                            IsActive = true,
                            MenuItemId = 63,
                            Name = "Pommes",
                            Price = 209m,
                            SortOrder = 3
                        },
                        new
                        {
                            Id = 52,
                            IsActive = true,
                            MenuItemId = 63,
                            Name = "Ris",
                            Price = 209m,
                            SortOrder = 4
                        });
                });

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Orders.Order", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Comment")
                        .HasColumnType("text");

                    b.Property<DateTime>("CreatedAtUtc")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("CustomerName")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("character varying(150)");

                    b.Property<string>("Email")
                        .HasMaxLength(254)
                        .HasColumnType("character varying(254)");

                    b.Property<string>("Phone")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<int>("RestaurantId")
                        .HasColumnType("integer");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<decimal>("TotalPrice")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.HasKey("Id");

                    b.HasIndex("RestaurantId");

                    b.ToTable("Orders");
                });

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Orders.OrderItem", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Comment")
                        .HasColumnType("text");

                    b.Property<int>("MenuItemId")
                        .HasColumnType("integer");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<int>("OrderId")
                        .HasColumnType("integer");

                    b.Property<int>("Quantity")
                        .HasColumnType("integer");

                    b.Property<decimal>("TotalPrice")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.Property<decimal>("UnitPrice")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.HasKey("Id");

                    b.HasIndex("OrderId");

                    b.ToTable("OrderItems");
                });

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Orders.OrderItemExtra", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int>("MenuItemExtraId")
                        .HasColumnType("integer");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<int>("OrderItemId")
                        .HasColumnType("integer");

                    b.Property<decimal>("PriceChange")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.HasKey("Id");

                    b.HasIndex("OrderItemId");

                    b.ToTable("OrderItemExtras");
                });

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Orders.OrderItemVariant", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int>("MenuItemVariantId")
                        .HasColumnType("integer");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("character varying(200)");

                    b.Property<int>("OrderItemId")
                        .HasColumnType("integer");

                    b.Property<decimal>("Price")
                        .HasPrecision(10, 2)
                        .HasColumnType("numeric(10,2)");

                    b.HasKey("Id");

                    b.HasIndex("OrderItemId")
                        .IsUnique();

                    b.ToTable("OrderItemVariants");
                });

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Restaurant", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Address")
                        .HasColumnType("text");

                    b.Property<string>("City")
                        .HasColumnType("text");

                    b.Property<DateTime>("CreatedAtUtc")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Email")
                        .HasColumnType("text");

                    b.Property<bool>("IsActive")
                        .HasColumnType("boolean");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Phone")
                        .HasColumnType("text");

                    b.Property<string>("PostalCode")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Restaurants");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Address = "Tittmyrvägen 1",
                            City = "Gävle",
                            CreatedAtUtc = new DateTime(2026, 9, 2, 0, 0, 0, 0, DateTimeKind.Utc),
                            IsActive = true,
                            Name = "Trödje Pizzeria",
                            Phone = "026-16 14 11",
                            PostalCode = "805 96"
                        });
                });

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Menu.MenuCategory", b =>
                {
                    b.HasOne("PizzeriaOrder.Api.Models.Restaurant", "Restaurant")
                        .WithMany("MenuCategories")
                        .HasForeignKey("RestaurantId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Restaurant");
                });

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Menu.MenuItem", b =>
                {
                    b.HasOne("PizzeriaOrder.Api.Models.Menu.MenuCategory", "MenuCategory")
                        .WithMany("MenuItems")
                        .HasForeignKey("MenuCategoryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("MenuCategory");
                });

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Menu.MenuItemExtra", b =>
                {
                    b.HasOne("PizzeriaOrder.Api.Models.Menu.MenuItem", "MenuItem")
                        .WithMany("Extras")
                        .HasForeignKey("MenuItemId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("MenuItem");
                });

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Menu.MenuItemVariant", b =>
                {
                    b.HasOne("PizzeriaOrder.Api.Models.Menu.MenuItem", "MenuItem")
                        .WithMany("Variants")
                        .HasForeignKey("MenuItemId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("MenuItem");
                });

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Orders.Order", b =>
                {
                    b.HasOne("PizzeriaOrder.Api.Models.Restaurant", "Restaurant")
                        .WithMany()
                        .HasForeignKey("RestaurantId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("Restaurant");
                });

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Orders.OrderItem", b =>
                {
                    b.HasOne("PizzeriaOrder.Api.Models.Orders.Order", "Order")
                        .WithMany("Items")
                        .HasForeignKey("OrderId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Order");
                });

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Orders.OrderItemExtra", b =>
                {
                    b.HasOne("PizzeriaOrder.Api.Models.Orders.OrderItem", "OrderItem")
                        .WithMany("Extras")
                        .HasForeignKey("OrderItemId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("OrderItem");
                });

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Orders.OrderItemVariant", b =>
                {
                    b.HasOne("PizzeriaOrder.Api.Models.Orders.OrderItem", "OrderItem")
                        .WithOne("Variant")
                        .HasForeignKey("PizzeriaOrder.Api.Models.Orders.OrderItemVariant", "OrderItemId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("OrderItem");
                });

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Menu.MenuCategory", b =>
                {
                    b.Navigation("MenuItems");
                });

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Menu.MenuItem", b =>
                {
                    b.Navigation("Extras");

                    b.Navigation("Variants");
                });

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Orders.Order", b =>
                {
                    b.Navigation("Items");
                });

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Orders.OrderItem", b =>
                {
                    b.Navigation("Extras");

                    b.Navigation("Variant");
                });

            modelBuilder.Entity("PizzeriaOrder.Api.Models.Restaurant", b =>
                {
                    b.Navigation("MenuCategories");
                });
#pragma warning restore 612, 618
        }
    }
}
